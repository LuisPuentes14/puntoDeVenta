...
namespace Proyecto_Metodologia.Reportes
{
    public partial class FrmReporteCompras : Form
    {
        ...
        private void btneExportar_Click(object sender, EventArgs e)
        {
            ...
            ExportarAExcelConClosedXML(datatable, ConfigurationManager.AppSettings["RutaReporteCompras"]);
        }
        ...
        private void CargarCompras(string filtro = "")
        {
            ...
            {
                query += "AND (p.Descripcion LIKE @Filtro OR C.FechaCompra LIKE @Filtro OR P.CodigoProducto LIKE @Filtro OR  P.Proveedor LIKE @Filtro OR P.PrecioCompra LIKE @Filtro OR P.PrecioUnitario LIKE @Filtro)";
            }
            ...
            foreach (DataGridViewRow fila in dgv.Rows)
            {
                if (!fila.IsNewRow)
                {
                    ...
                    for (int i = 0; i < dgv.Columns.Count; i++)
                    {
                        ...
                        {
                            case "PrecioUnitario":
                                nuevaFila[i] = float.TryParse(valor, out float cant) ? cant : 0;
                                break;
                            case "PrecioCompra":
